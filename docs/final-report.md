...
現代線上會議常見的問題，不只是會後忘記討論內容，而是會議進行中就已經來不及參照正確脈絡。許多討論其實圍繞 Google Docs、需求文件、企劃書或課程文件進行；當大家一邊討論一邊修改文件時，使用者需要同時聽懂對方的意思、對照文件內容、記下目前結論、判斷下一步，甚至在對方要求時同步修改文件或確認待辦事項。

目前許多 AI 會議工具已能做到錄音、轉錄、摘要與會後搜尋，但這些功能多半在會議結束後才發揮價值。對實際會議情境而言，很多關鍵時刻發生在當下。例如，使用者可能需要馬上確認「文件目前寫的是哪一版？」、「剛剛討論的限制有沒有寫進 Google Doc？」、「這個決定後面還要追問誰？」。如果 AI 只能在會後整理紀錄，就無法協助使用者處理這些即時 referencing 與 follow-up 的需求。
...
1. **文件脈絡斷裂**：使用者需要一邊聽會議，一邊回頭對照 Google Docs 文件內容，容易漏掉目前討論對應到文件哪一段。
2. **追問與 follow-up 不足**：使用者知道對方的說法還不夠清楚，但很難在當下整理出精準的追問、下一步或責任分工。
3. **回覆困難**：使用者在會議中不知道下一句該怎麼回，或擔心自己回答得太快而做出不必要的承諾。
...
然而，本專題觀察到，單純再做一個 AI notetaker 並沒有明顯差異。轉錄、摘要、action items 已逐漸成為基礎功能。若要做出新的價值，重點應先放在會議進行中正在使用的 Google Docs 文件脈絡，讓 AI 能即時 reference 共同文件；接著才是在此基礎上提供追問、回覆、notes/actions 等 follow-up support。

| 工具                         | 主要強項                                          | 與 emeet 的差異                                                                   |
| -------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------- |
| Otter.ai                   | 即時轉錄、會議摘要、AI Chat、action items                | Otter.ai 強在跨會議筆記與會後工作流；emeet 更重視會議中的 Google Docs referencing 與後續追問支援。                |
| Fireflies.ai               | 會議錄音、轉錄、摘要、action items、AskFred               | Fireflies.ai 通常需要以機器人形式加入會議；emeet 採 macOS 本機音訊擷取，不綁定特定會議平台。                   |
| Fathom                     | AI notetaker、摘要模板、action items、CRM 工作流        | Fathom 偏向會後紀錄與團隊流程；emeet 聚焦會中共同文件脈絡與可控文件操作。                                  |
| Granola                    | 無 bot AI notepad，結合手寫筆記與 AI notes             | Granola 的方向與 emeet 較接近，但 emeet 進一步實驗 Google Docs referencing、追問、模型供應商抽象與文件編輯。    |
| Krisp AI Meeting Assistant | 轉錄、會議紀錄、action items、噪音處理                     | Krisp 強在音訊處理與會議紀錄；emeet 更重視即時互動與可替換模型架構。                                      |
| Notion AI Meeting Notes    | 會議轉錄後整合進 Notion workspace                     | Notion 適合已在 Notion 工作流中的使用者；emeet 目前以 Google Docs 作為即時文件 referencing MVP。          |
...
許多會議其實圍繞文件進行，例如企劃書、合約、課程文件、需求規格、專案紀錄等。emeet 將會議內容和 Google Docs 串接，讓 AI 不只理解語音，也能理解正在討論的文件脈絡。第一版的重點，是讓會議中的 notes、actions、回覆與追問都能參照同一份共同文件，而不是只依賴零散逐字稿。

### 2. Follow-up support

在文件脈絡建立之後，emeet 的另一個價值是協助使用者把討論往下一步推進。它希望在使用者還在對話時，就能根據最新逐字稿與文件內容產生可用的追問、保守回覆、會議紀錄與下一步行動，而不是等會議結束後才整理內容。
...
本專題的目標是完成一個可實際展示的即時會議輔助原型。從產品優先順序來看，第一個目標是驗證「Google Docs 會議即時 referencing」：使用者在會議中可以連接正在討論的 Google Docs，讓系統讀取文件脈絡、同步 live notes，並在明確指令下做有限度文件操作。

第二個目標是在文件脈絡之上提供 follow-up support。系統需要能從 macOS 擷取會議音訊，將其轉成逐字稿，再把逐字稿與 Google Docs 內容一起送入助理管線，產生追問、保守回覆、會議紀錄與下一步行動。換言之，emeet 不是先做一個獨立摘要工具，再附加文件功能；它的展示重點是把「會議中的共同文件」變成 AI 輔助的 grounding，再由此支援後續討論。

目前完成的功能包括：

1. 連接 Google Docs，讀取文件脈絡、更新 live notes，並可由語音觸發有限度文件編輯。
2. 將 Google Docs 標題、摘要與 snippets 放入 assistant context，讓回覆、追問與會議筆記能 reference 文件內容。
3. 同時擷取麥克風與系統音訊。
4. 將音訊轉成 16 kHz mono PCM16，約每 100 ms 傳送至後端。
5. 後端依 speech window 進行語音分段。
6. 支援 `faster-whisper`、`mlx-whisper` 與 Breeze ASR 25 等語音轉文字路徑。
7. 在 UI 中顯示 `Self`、`Other` 或 `Speaker 1`、`Speaker 2` 等說話者標籤。
8. 使用 `Follow-up questions` 產生有助於釐清需求、限制、風險與下一步的問題。
9. 使用 `What should I say?` 產生保守、自然、可直接說出口的回覆。
10. 每 30 秒根據新增逐字稿與文件脈絡更新會議紀錄與下一步行動。
11. 支援 Markdown 形式的會議紀錄匯出。
12. 支援助理模型供應商、模型與 thinking 設定切換。
13. 使用 SQLite 儲存會議、逐字稿、助理執行結果、會議筆記與行動項目。
...
會中輔助和會後摘要對系統的要求不同。會後摘要可以等待較長時間，但要求完整、穩定、可追溯；會中 referencing 和 follow-up 則要求能快速掌握最新逐字稿與目前文件內容，即使資訊不完整，也要避免胡亂推論。

因此 emeet 採用不同的更新節奏：

* 音訊傳輸：每 100 ms 傳送 PCM16 frame。
* 語音轉文字：後端累積 speech window 後產生 final transcript segment。
* Google Docs referencing：連接文件後讀取標題、摘要、snippets，供助理請求使用。
* 回覆與追問：由使用者按鈕觸發，並參照最近逐字稿與文件脈絡。
* 會議紀錄：每 30 秒低頻自動更新。
...
| 文件脈絡層 | 連接 Google Docs 後讀取並整理文件 snapshot | 文件標題、摘要、headings、sections、paragraph snippets | 讓會議助理能 reference 正在討論的共同文件。 |
| 即時建議層 | 目前由 `What should I say?` 與 `Follow-up questions` 按鈕觸發；未來可在語意邊界或 6 到 10 秒心跳預先更新候選 | 最近逐字稿、目前議題、未解問題、rolling summary、Google Docs 脈絡 | 服務當下回話與追問，不應等待 30 秒摘要週期。 |
...
`What should I say?` 是 Google Docs referencing 之後的 follow-up support。它會根據最近逐字稿、目前會議脈絡，以及已連接文件的標題、摘要與 snippets，產生 2 到 3 個短句形式的回覆建議。這些建議的設計原則是：
...
### 6.5 追問問題與 follow-up support

`Follow-up questions` 會根據會議內容與 Google Docs 脈絡產生幾個適合當下提出的追問。這些問題通常用來釐清：
...
Google Docs Live Notes 是本專題第一個產品展示目標。當 Google Docs 已連線時，emeet 會將自動產生的 notes 與 actions 更新到文件中的 `emeet Live Notes` 區塊。這是目前文件 referencing 與共編的 MVP：會議進行中，文件會同步呈現目前討論重點與下一步行動，後續的回覆、追問與筆記也都能參照這份共同文件脈絡。
...
另一個重要收穫是，產品定位會影響技術架構。一開始若把 emeet 定義成會後摘要工具，可能只需要錄音、轉錄與摘要。但當目標改成 Google Docs 即時 referencing 的會議副駕後，系統就必須重視文件 snapshot、文件結構 mapping、低延遲逐字稿、會中按鈕互動、保守回覆、追問生成、文件脈絡與可控編輯。這讓專題的技術範圍更複雜，但也更有探索價值。
